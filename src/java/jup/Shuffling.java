...
import jup.crypto.AnonymouslyEncryptedData;
import jup.crypto.Crypto;
import jup.db.DbClause;
import jup.db.DbIterator;
import jup.db.DbKey;
import jup.db.DbUtils;
import jup.db.VersionedEntityDbTable;
import jup.util.Convert;
import jup.util.Listener;
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_JUP, Constants.COIN_SYMBOL, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_JUP, Constants.COIN_SYMBOL, block.getHeight());
        }
