...
public abstract class ShufflingTransaction extends TransactionType {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (holdingType == HoldingType.NXT) {
                if (amount < Constants.SHUFFLING_DEPOSIT_NQT || amount > Constants.MAX_BALANCE_NQT) {
                    throw new JupException.NotValidException("Invalid JUP amount " + amount
                ...
                if (asset == null) {
                    throw new JupException.NotCurrentlyValidException("Unknown asset " + Long.toUnsignedString(attachment.getHoldingId()));
                }
                if (amount <= 0 || amount > asset.getInitialQuantityQNT()) {
                    throw new JupException.NotValidException("Invalid asset quantity " + amount);
                }
            } else if (holdingType == HoldingType.CURRENCY) {
                Currency currency = Currency.getCurrency(attachment.getHoldingId());
                CurrencyType.validate(currency, transaction);
                if (!currency.isActive()) {
                    throw new JupException.NotCurrentlyValidException("Currency is not active: " + currency.getCode());
                }
                if (amount <= 0 || amount > Constants.MAX_CURRENCY_TOTAL_SUPPLY) {
                    throw new JupException.NotValidException("Invalid currency amount " + amount);
                }
            } else {
            ...
            if (attachment.getParticipantCount() < Constants.MIN_NUMBER_OF_SHUFFLING_PARTICIPANTS
                ...
                throw new JupException.NotValidException(String.format("Number of participants %d is not between %d and %d",
            ...
            if (attachment.getRegistrationPeriod() < 1 || attachment.getRegistrationPeriod() > Constants.MAX_SHUFFLING_REGISTRATION_PERIOD) {
                throw new JupException.NotValidException("Invalid registration period: " + attachment.getRegistrationPeriod());
            }
        }
    ...
    public static final TransactionType SHUFFLING_REGISTRATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (shuffling == null) {
                throw new JupException.NotCurrentlyValidException("Shuffling not found: " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            ...
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new JupException.NotCurrentlyValidException("Shuffling state hash doesn't match");
            }
            if (shuffling.getStage() != Shuffling.Stage.REGISTRATION) {
                throw new JupException.NotCurrentlyValidException("Shuffling registration has ended for " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            if (shuffling.getParticipant(transaction.getSenderId()) != null) {
                throw new JupException.NotCurrentlyValidException(String.format("Account %s is already registered for shuffling %s",
            ...
            if (Jup.getBlockchain().getHeight() + shuffling.getBlocksRemaining() <= attachment.getFinishValidationHeight(transaction)) {
                throw new JupException.NotCurrentlyValidException("Shuffling registration finishes in " + shuffling.getBlocksRemaining() + " blocks");
            }
        }
    ...
    public static final TransactionType SHUFFLING_PROCESSING = new ShufflingTransaction() {
        ...
        @Override
        Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer) throws JupException.NotValidException {
            return new Attachment.ShufflingProcessing(buffer);
        }

        @Override
        Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws JupException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (shuffling.getStage() != Shuffling.Stage.PROCESSING) {
                throw new JupException.NotCurrentlyValidException(String.format("Shuffling %s is not in processing stage",
            ...
            if (participant == null) {
                throw new JupException.NotCurrentlyValidException(String.format("Account %s is not registered for shuffling %s",
                        Long.toUnsignedString(transaction.getSenderId()), Long.toUnsignedString(shuffling.getId())));
            }
            if (!participant.getState().canBecome(ShufflingParticipant.State.PROCESSED)) {
                throw new JupException.NotCurrentlyValidException(String.format("Participant %s processing already complete",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            if (participant.getAccountId() != shuffling.getAssigneeAccountId()) {
                throw new JupException.NotCurrentlyValidException(String.format("Participant %s is not currently assigned to process shuffling %s",
                        Long.toUnsignedString(participant.getAccountId()), Long.toUnsignedString(shuffling.getId())));
            }
            if (participant.getNextAccountId() == 0) {
                throw new JupException.NotValidException(String.format("Participant %s is last in shuffle",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
            ...
            if (data != null) {
                if (data.length != participant.getIndex() + 1 && data.length != 0) {
                    throw new JupException.NotValidException(String.format("Invalid number of encrypted data %d for participant number %d",
                            data.length, participant.getIndex()));
                }
                byte[] previous = null;
                for (byte[] bytes : data) {
                    if (bytes.length != 32 + 64 * (shuffling.getParticipantCount() - participant.getIndex() - 1)) {
                        throw new JupException.NotValidException("Invalid encrypted data length " + bytes.length);
                    }
                    if (previous != null && Convert.byteArrayComparator.compare(previous, bytes) >= 0) {
                        throw new JupException.NotValidException("Duplicate or unsorted encrypted data");
                    }
                    previous = bytes;
                }
            }
    ...
    public static final TransactionType SHUFFLING_RECIPIENTS = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (participant.getNextAccountId() != 0) {
                throw new JupException.NotValidException(String.format("Participant %s is not last in shuffle",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            if (!participant.getState().canBecome(ShufflingParticipant.State.PROCESSED)) {
                throw new JupException.NotCurrentlyValidException(String.format("Participant %s processing already complete",
            ...
            if (recipientPublicKeys.length != shuffling.getParticipantCount() && recipientPublicKeys.length != 0) {
                throw new JupException.NotValidException(String.format("Invalid number of recipient public keys %d", recipientPublicKeys.length));
            }
            ...
            for (byte[] recipientPublicKey : recipientPublicKeys) {
                if (!Crypto.isCanonicalPublicKey(recipientPublicKey)) {
                    throw new JupException.NotValidException("Invalid recipient public key " + Convert.toHexString(recipientPublicKey));
                }
                if (!recipientAccounts.add(Account.getId(recipientPublicKey))) {
                    throw new JupException.NotValidException("Duplicate recipient accounts");
                }
            }
        }
    ...
    public static final TransactionType SHUFFLING_VERIFICATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (shuffling.getStage() != Shuffling.Stage.VERIFICATION) {
                throw new JupException.NotCurrentlyValidException("Shuffling not in verification stage: " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            ShufflingParticipant participant = shuffling.getParticipant(transaction.getSenderId());
            if (participant == null) {
                throw new JupException.NotCurrentlyValidException(String.format("Account %s is not registered for shuffling %s",
            ...
            if (!participant.getState().canBecome(ShufflingParticipant.State.VERIFIED)) {
                throw new JupException.NotCurrentlyValidException(String.format("Shuffling participant %s in state %s cannot become verified",
            ...
            if (participant.getIndex() == shuffling.getParticipantCount() - 1) {
                throw new JupException.NotValidException("Last participant cannot submit verification transaction");
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new JupException.NotCurrentlyValidException("Shuffling state hash doesn't match");
    ...
    public static final TransactionType SHUFFLING_CANCELLATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws JupException.ValidationException {
            ...
            if (cancellingAccountId == 0 && !shuffling.getStage().canBecome(Shuffling.Stage.BLAME)) {
                throw new JupException.NotCurrentlyValidException(String.format("Shuffling in state %s cannot be cancelled", shuffling.getStage()));
            }
            if (cancellingAccountId != 0 && cancellingAccountId != shuffling.getAssigneeAccountId()) {
                throw new JupException.NotCurrentlyValidException(String.format("Shuffling %s is not currently being cancelled by account %s",
            ...
            if (!participant.getState().canBecome(ShufflingParticipant.State.CANCELLED)) {
                throw new JupException.NotCurrentlyValidException(String.format("Shuffling participant %s in state %s cannot submit cancellation",
                        Long.toUnsignedString(attachment.getShufflingId()), participant.getState()));
            }
            if (participant.getIndex() == shuffling.getParticipantCount() - 1) {
                throw new JupException.NotValidException("Last participant cannot submit cancellation transaction");
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new JupException.NotCurrentlyValidException("Shuffling state hash doesn't match");
            ...
            if (dataProcessingTransaction == null) {
                throw new JupException.NotCurrentlyValidException("Invalid data transaction full hash");
            }
            ...
            if (!Arrays.equals(shufflingProcessing.getHash(), attachment.getHash())) {
                throw new JupException.NotValidException("Blame data hash doesn't match processing data hash");
            }
            byte[][] keySeeds = attachment.getKeySeeds();
            if (keySeeds.length != shuffling.getParticipantCount() - participant.getIndex() - 1) {
                throw new JupException.NotValidException("Invalid number of revealed keySeeds: " + keySeeds.length);
            }
            for (byte[] keySeed : keySeeds) {
                if (keySeed.length != 32) {
                    throw new JupException.NotValidException("Invalid keySeed: " + Convert.toHexString(keySeed));
                }
            }
        }
